...
* set seed to ensure replicability of estimates from simluations
execseed = 82877;
...
set      t   time step representing one year    / 0*10 /;
alias(t, tt);

*set 1 if we want to execute_unload the results
$setglobal sendresults 1

*================================================================================
*Choose which shock(s) to activate by setting the corresponding shock equal to 1
*================================================================================
*Define active Shocks (equal to zero by default)

$if not set MDAshock  $setglobal MDAshock  0
$if not set FMPshock  $setglobal FMPshock  0
$if not set TFPshock  $setglobal TFPshock  0
$if not set RoWtrade  $setglobal RoWtrade  0

*SI appendix: alternative labor supply elasticity specifications (both off by
*default, i.e. the baseline hfsupel=100 assumption used throughout the main text).
*SI_LabElas1: set elasticity to 1 instead of 100.
*SI_LabElasINF: elasticity declining in aggregate infection status.
$if not set SI_LabElas1    $setglobal SI_LabElas1    0
$if not set SI_LabElasINF  $setglobal SI_LabElasINF  0

*Define Shock parameters (equal to zero by default)
parameter MDAshock equal to 1 if simulating MDA shock  / %MDAshock% /
          FMPshock equal to 1 if simulating FMP shock  / %FMPshock% /
          TFPshock equal to 1 if simulating TFP shock  / %TFPshock% /
          SI_LabElas1 equal to 1 if using SI labor elasticity = 1 specification / %SI_LabElas1% /
          SI_LabElasINF equal to 1 if using SI labor elasticity declining-in-infection specification / %SI_LabElasINF% /
;
...
*Define Trade parameter (equal to 1 for preferred specification of fish traded RoW)
parameter RoWtrade equal to 1 if Fish is traded RoW and zero if traded Local
;
RoWtrade = %RoWtrade% ;
...
* 1. Create a single variable to hold the shock part of the filename
$setglobal shock_id ""

* 2. Append the shock names if they are active
$ifthen %MDAshock% == 1
$setglobal shock_id "%shock_id%_MDA"
$endif

$ifthen %FMPshock% == 1
$setglobal shock_id "%shock_id%_FMP"
$endif

$ifthen %TFPshock% == 1
$setglobal shock_id "%shock_id%_TFP"
$endif

$ifthen %SI_LabElas1% == 1
$setglobal shock_id "%shock_id%_LabElas1"
$endif

$ifthen %SI_LabElasINF% == 1
$setglobal shock_id "%shock_id%_LabElasINF"
$endif
...
* goods subsets
     gtv(g)    goods tradable in the village / ser, ret, meat, crop, fish /
...
* Factor supply
     ...
     hfsupel_ts(f,h,t)   factor supply elasticity actually used in period t (SI_LabElasINF tracking)
     agg_infectrate_sims_ts(t)  population-weighted aggregate infection rate entering period t's labor supply decision (SI_LabElasINF tracking)
...
parameter theta_fish1_dr(g,f,h)       coefficient to represent share of the stocks value added
          ...
          x_frac_of_K_ts(t)           fish stock as fraction of carrying capacity
          x_frac_of_K_d(t)           difference across t in fish stock as fraction of carrying capacity
          x_frac_of_K_pc(t)           percent change in fish stock as fraction of carrying capacity
;
...
parameter qcshare(h,g) share of household h in total consumption of g
          netexpsh(g) net export share of a good out of the zoi
          qpshare(h,g) share of household h in production of g
;
...
parameter feinc_dr(h)  income from factor endowments in the household
...
**d_sc is calibrated after the baseline solve (see below), from stockvar.l
*rather than stock_dr, to avoid seeding a floating-point mismatch between the
*two -- see the comment at that assignment for the full explanation.
...
*** Labor Supply
hfsupel("LABOR",h) = 100 ;
*SI appendix: elasticity=1 specification overrides the baseline 100 above.
hfsupel("LABOR",h)$(SI_LabElas1 = 1) = 1 ;
...
*Use stockvar.l (BL_MODEL_X's actual solved value) to seed the loop, not
*stock_dr (the pre-solve calibration target). Every other baseline-converged
*quantity (P.l, QP.l, etc.) is already internally consistent with stockvar.l,
*since BL_MODEL_X's own equations (EQ_QVACOBB_stockvar) are built on stockvar,
*not stock_dr -- so stockvar.l is the correct anchor for the loop; see the
*d_sc calibration below, which is deliberately computed from this same value
*(after the baseline solve) rather than from stock_dr beforehand, to avoid
*seeding a floating-point mismatch between the two.
stock("fish") = stockvar.l("fish") ;

*x_step_ts is the fish-stock state variable: the stock at the start of period t,
*which determines that period's search costs and harvest (same-period recursion,
*X(t+1) = X(t) + growth(X(t)) - harvest(t)). x_ts is a separate, derived
*reporting series giving the stock as of the end of period t, so that "Year t"
*fish stock lines up with how every other outcome is reported.
parameter x_step_ts(gstk,t) "fish stock state: level at the start of period t" ;
x_step_ts("fish",t) = stockvar.l("fish") ;
x_ts("fish","0") = stockvar.l("fish") ;

*Recalibrate d_sc from the baseline's actual solved stock (stockvar.l), not
*the pre-solve target (stock_dr), so the loop's first idsh recomputation
*(d_sc/stock(gstk)**n_sc, using stock(gstk)=stockvar.l set just above) exactly
*inverts this same multiplication instead of leaving a tiny residual from
*using two different (if theoretically identical) reference values.
d_sc(gstk,gg,h) = (stockvar.l(gstk)**n_sc)*idsh_dr(gstk,gg,h);
...
infectrate_ts("0") = sum(h,infectrate_hh_ts(h,"0")*shhh(h)) ;
...
*SI appendix: labor elasticity declining in aggregate infection status.
*hfsupel itself (used inside EQ_HFSUP_SIMS) can't be time-indexed there -- the
*equation is defined once, solved fresh each loop iteration -- so, exactly
*like Infectrate_hh_sims(h) already does for the productivity-loss term in
*that same equation, hfsupel is a plain (non-time-indexed) parameter that
*gets reassigned here, each iteration, before the solve. hfsupel_ts tracks
*what was actually used each period, for output/diagnostics. Uses
*Infectrate_hh_sims(h) -- the same, already-available, previous-period
*infection rate the equation's productivity term uses -- so this introduces
*no new timing/circularity issue beyond what's already in the model.
agg_infectrate_sims_ts(t)$(SI_LabElasINF = 1) = sum(h,shhh(h)*Infectrate_hh_sims(h)) ;
hfsupel_ts("LABOR",h,t)$(SI_LabElasINF = 1) = max(1, 100 - 100*agg_infectrate_sims_ts(t)) ;
hfsupel("LABOR",h)$(SI_LabElasINF = 1) = hfsupel_ts("LABOR",h,t) ;
...
*function, applied to the start-of-period stock and this period's harvest).
*Units in millions of kilos.

x_step_ts(gstk,t+1) = x_step_ts(gstk,t) + gr(gstk)*x_step_ts(gstk,t)*(1-x_step_ts(gstk,t)/k_dr(gstk)) - sum(h,QP.l(gstk,h))*kilo_conversion(gstk) ;

*Redefine the model stock 'parameter' for use in next period's LEWIE solve.
         stock(gstk) = x_step_ts(gstk,t+1);

*Reporting series: stock as of the end of period t (start-of-period state plus
*this period's growth and harvest), so x_ts(gstk,t) lines up with "Year t" the
*same way every other outcome does. Computed directly rather than read from
*x_step_ts(gstk,t+1), since t="10" has no t+1 slot in the t/0*10/ domain.
x_ts(gstk,t) = x_step_ts(gstk,t) + gr(gstk)*x_step_ts(gstk,t)*(1-x_step_ts(gstk,t)/k_dr(gstk)) - sum(h,QP.l(gstk,h))*kilo_conversion(gstk) ;
...
*Now pass on values from the solution to the state variables in time t.
         ...
         infectrate_ts(t) = sum(h,infectrate_hh_ts(h,t)*shhh(h)) ;
...
*calculate x_frac_of_K for each t
x_frac_of_K_ts(t)$k_dr("fish") = 100* x_ts("fish",t)/k_dr("fish") ;

parameter hhpop(h) ;

* 1. Calculate the total population for each household group
* Assuming xlnhh is the number of households and xlhhsize is the average size [cite: 103, 121]
hhpop(h) = xlnhh(h) * xlhhsize(h);

* 2. Setup Discount Factors (as per your current logic)
...
infectrate_hh_d(h,t)$infectrate_hh_ts(h,"0")=infectrate_hh_ts(h,t)-infectrate_hh_ts(h,"0");
fishlabtime_d(t)$fishlabtime_ts("0")=fishlabtime_ts(t)-fishlabtime_ts("0");
bigr0_d(t)$bigr0_ts("0")=bigr0_ts(t)-bigr0_ts("0");
harvestkg_d(gstk,t)=harvestkg_ts(gstk,t)-harvestkg_ts(gstk,"0");
x_frac_of_K_d(t)=x_frac_of_K_ts(t) - x_frac_of_K_ts("0");

**Percentage changes from baseline
ttqp_pc(t)$ttqp_ts("0")=100 *ttqp_d(t) / ttqp_ts("0") ;
ry_pc(h,t)$ry_ts(h,"0")=100 *ry_d(h,t) / ry_ts(h,"0") ;
infectrate_hh_pc(h,t)$infectrate_hh_ts(h,"0")=100 *infectrate_hh_d(h,t) / infectrate_hh_ts(h,"0");
fishlabtime_pc(t)$fishlabtime_ts("0")=100 *fishlabtime_d(t) / fishlabtime_ts("0");
bigr0_pc(t)$bigr0_ts("0")=100 *bigr0_d(t) / bigr0_ts("0");
harvestkg_pc(gstk,t) = 100* (harvestkg_d(gstk,t) ) /harvestkg_ts(gstk,"0") ;
x_frac_of_K_pc(t) = 100* x_frac_of_K_d(t) / x_frac_of_K_ts("0") ;

*Retain exact-zero percent-change results as explicit records (GDX's sparse
*storage silently omits any parameter entry that computes to exactly zero,
*which a positional read in MATLAB can misinterpret as a missing year).
bigr0_pc(t)$(bigr0_ts("0") and (bigr0_pc(t) = 0)) = EPS ;
ry_pc(h,t)$(ry_ts(h,"0") and (ry_pc(h,t) = 0)) = EPS ;
infectrate_hh_pc(h,t)$(infectrate_hh_ts(h,"0") and (infectrate_hh_pc(h,t) = 0)) = EPS ;
fishlabtime_pc(t)$(fishlabtime_ts("0") and (fishlabtime_pc(t) = 0)) = EPS ;
ttqp_pc(t)$(ttqp_ts("0") and (ttqp_pc(t) = 0)) = EPS ;
harvestkg_pc(gstk,t)$(harvestkg_pc(gstk,t) = 0) = EPS ;
x_frac_of_K_pc(t)$(x_frac_of_K_pc(t) = 0) = EPS ;

$ifthen %sendresults% == 1
execute_unload "./output/%trade%%shock_id%%rts%.gdx" ;
$endif
